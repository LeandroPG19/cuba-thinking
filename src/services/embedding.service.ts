...
const __freqCache = new Map<string, { freq: Map<string, number>, norm: number }>();

export function keywordSimilarity(textA: string, textB: string): number {
  if (textA === textB) return 1.0;
  if (!textA.trim() || !textB.trim()) return 0.0;

  let dataA = __freqCache.get(textA);
  if (!dataA) {
    const tokensA = tokenize(textA);
    const freqA = frequencyMap(tokensA);
    let normA = 0;
    freqA.forEach((countA) => {
      normA += countA * countA;
    });
    dataA = { freq: freqA, norm: normA };
    if (__freqCache.size > 1000) {
      // Prevent unbounded growth
      const firstKey = __freqCache.keys().next().value;
      if (firstKey) __freqCache.delete(firstKey);
    }
    __freqCache.set(textA, dataA);
  }

  let dataB = __freqCache.get(textB);
  if (!dataB) {
    ...
    dataB = { freq: freqB, norm: normB };
    if (__freqCache.size > 1000) {
      // Prevent unbounded growth
      const firstKey = __freqCache.keys().next().value;
      if (firstKey) __freqCache.delete(firstKey);
    }
    __freqCache.set(textB, dataB);
  }

  if (dataA.freq.size === 0 || dataB.freq.size === 0) return 0.0;

  let dotProduct = 0;
  dataA.freq.forEach((countA, token) => {
    const countB = dataB.freq.get(token);
    if (countB !== undefined) {
      dotProduct += countA * countB;
    }
  });

  if (dataA.norm === 0 || dataB.norm === 0) return 0;
  return dotProduct / (Math.sqrt(dataA.norm) * Math.sqrt(dataB.norm));
}
